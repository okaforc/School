...
class Collinear {
	 ...
	 *         Order of growth: N^3
	 *
	 *         Explanation: Triple nested for loop.
	 */
	static int countCollinear(int[] a1, int[] a2, int[] a3) {
		int total = 0, y1 = 1, y2 = 2, y3 = 3;	// define total and points y1-y3

		for (int i = 0; i < a1.length; i++) {
			for (int j = 0; j < a2.length; j++) {
				for (int k = 0; k < a3.length; k++) {
					if ((a1[i] * (y2 - y3) + a2[j] * (y3 - y1) + a3[k] * (y1 - y2)) == 0) {	
						// if the formula holds true for three numbers
						// add to the total number of valid lines
						total++;
					}
				}
			}
	 ...
	 *
	 *         Explanation: Two linear for loops looping over N, with binarySearch (in log N) being implemented every pass. 
	 *
	 *
	 */
	static int countCollinearFast(int[] a1, int[] a2, int[] a3) {
		int total = 0, y1 = 1, y2 = 2, y3 = 3;

		// sort array for binary search
		sort(a3);

		/**
		 * if binarySearch can find a suitable third element in the third array given a manipulated formula:
		 * x3 = -(x1(y2-y3) + x2(y3-y1))/ y1-y2
		 * then the total number of valid lines can be increased
		*/

		for (int i = 0; i < a1.length; i++) {
			for (int j = 0; j < a2.length; j++) {
				if (binarySearch(a3, -(a1[i] * (y2 - y3) + a2[j] * (y3 - y1)) / (y1 - y2))) {
					total++;
				}
			}
		}
	 ...
	 *
	 *         Explanation: Array is halved a certain number of times, and so scales with the amount of inputs.
	 *
	 */
	static boolean binarySearch(int[] a, int x) {
		int mid, hi = a.length - 1, lo = 0;
