...
struct bloom *bloom_new(int size) {
    struct bloom *bl = calloc((size / sizeof(0ULL) + 1), sizeof(struct bloom));
...
int bloom_lookup(struct bloom *bloomset, char *item) {
    int seeds[] = {BLOOM_HASH1, BLOOM_HASH2};
    int nhashes = 2; // number of hash seeds
    for (int i = 0; i < nhashes; i++) {
        int hash = hash_string(item, seeds[i], bloomset->size);
        if (bitset_lookup(bloomset->bitset, hash) == 0) {
            return 0;
        }
    }
    return 1;
...
void bloom_add(struct bloom *bloomset, char *item) {
    int hash = hash_string(item, BLOOM_HASH1, bloomset->size);
    bitset_add(bloomset->bitset, hash);
    hash = hash_string(item, BLOOM_HASH2, bloomset->size);
    bitset_add(bloomset->bitset, hash);
}
