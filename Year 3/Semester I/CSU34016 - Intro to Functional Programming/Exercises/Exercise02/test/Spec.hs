...
lawTests :: TF.Test
lawTests
 = testGroup "\nPart 2 - laws (30 marks, worth 15 BB points)\n"
    [ law1Tests
    , law2Tests
    , law3Tests
    , law4Tests
...
law1Tests
 = testGroup "\nLaw 1 tests (6 marks)\n"
    [ testCase "1*2 is 2*1 [2 marks]"
        (law1 (Mul (Val 1) (Val 2)) @?= Just (Mul (Val 2) (Val 1)))
    , testCase "Cannot handle 1+2 [2 marks]"
        (law1  (Add (Val 1) (Val 2)) @?= Nothing)
    , testCase "x+y=y+x [2 marks]"
        (eval dx42y99 (fromJust $ law1  (Mul vx vy))
          @?= eval dx42y99 (Mul vy vx))
    ]

--   (x + y) + z  =  x + (y + z)  Law 2
law2Tests
 = testGroup "\nLaw 2 tests (6 marks)\n"
    [ testCase "(1+2)+3 is 1+(2+3) [2 mark]"
        ( law2 (Add (Add (Val 1) (Val 2)) (Val 3))
          @?= Just (Add (Val 1) (Add (Val 2) (Val 3))) )
    , testCase "Cannot handle 1/(2/3) [2 mark]"
        ( law2  (Dvd (Val 1) (Dvd (Val 2) (Val 3))) @?= Nothing)
    , testCase "(x+y)+7=x+(y+7) [2 mark]"
...
law3Tests
 = testGroup "\nLaw 3 tests (6 marks)\n"
    [ testCase "(1/2)/3 is 1/(2*3) [2 marks]"
        ( law3 (Dvd (Dvd (Val 1) (Val 2)) (Val 3))
          @?= Just (Dvd (Val 1) (Mul (Val 2) (Val 3))) )
    , testCase "Cannot handle (2-3)-1 [2 mark]"
        ( law3  (Sub (Sub (Val 2) (Val 3)) (Val 1) ) @?= Nothing)
    , testCase "(x/y)/7 = x/(y*7) [2 mark]"
        (eval dx42y99 (fromJust $ law3 (Dvd (Dvd vx vy) (Val 7)) )
          @?= eval dx42y99 (Dvd vx (Mul vy (Val 7))) )
    ]

--   (x + y)*(x - y)  =  x*x - y*y     Law 4
law4Tests
 = testGroup "\nLaw 4 tests (12 marks)\n"
    [ testCase "Cannot handle 99*99-42*42 [4 marks]"
        ( law4 (Sub (Mul (Val 99) (Val 99)) (Mul (Val 42) (Val 42)))
          @?= Nothing )
    , testCase "(99+42)*(99-42) = (99*42)-(99*42) [4 marks]"
        ( law4  (Mul (Add (Val 99) (Val 42)) (Sub (Val 99) (Val 42))) 
          @?= Just (Sub (Mul (Val 99) (Val 99)) (Mul (Val 42) (Val 42)))) 
    , testCase "(y+x)*(y-x) is y*y-x*x [4 marks]"
