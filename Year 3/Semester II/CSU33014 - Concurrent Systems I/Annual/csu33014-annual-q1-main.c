...
int get_random(int range, int must_be_positive) {
  int num = random();
  int sign = random();

  num = num % range;
  // if must positive, then ensure no zero values
  if ( must_be_positive ) {
    if ( num == 0 ) {
      num = 1;
    }
...
void test_routine_1() {
  ...
  Q1_routine_1(out_correct, b, size);
  Q1_vectorized_1(out_vectorized, b, size);
...
void test_routine_2() {
  int size = get_random(10, 1) + 10; // get a random size > 10
  float * a = new_random_float_array(size, 1, 16);
  float out_correct, out_vectorized;
  double diff;

  out_vectorized = Q1_vectorized_2(a, size);
  out_correct = Q1_routine_2(a, size);
...
void test_routine_3() {
  int size = get_random(2048, 1) + 10; // get a random size > 10
  float * out_correct_a = new_random_float_array(size, 1, 0);
  float * out_vectorized_a = clone_float_array(out_correct_a, size);
  float * out_correct_b = new_random_float_array(size, 1, 0);
  float * out_vectorized_b = clone_float_array(out_correct_b, size);
  double diff;

  // fprintf(stderr, "out_correct %p, out_vectorized %p, b %p, size %d\n", out_correct, out_vectorized, b, size);

  Q1_routine_3(out_correct_a, out_correct_b, size);
  Q1_vectorized_3(out_vectorized_a, out_vectorized_b, size);

  diff = diff_square(out_correct_a, out_vectorized_a, size) +
    diff_square(out_correct_b, out_vectorized_b, size);
  free(out_correct_a);
  free(out_vectorized_a);
  free(out_correct_b);
  free(out_vectorized_b);
...
void test_routine_4() {
  ...
  float * correct_a = new_random_float_array(size, 1, 0);
  float * vectorized_a = clone_float_array(correct_a, size);

  Q1_routine_4(correct_a);
  Q1_vectorized_4(vectorized_a);
  
  diff = diff_square(correct_a, vectorized_a, size);
...
void test_routine_5() {
  ...
  unsigned char * b2_char = (unsigned char *) b2;
  unsigned char * c2_char = (unsigned char *) c2;
  unsigned char * out_correct_char = (unsigned char *) out_correct;
  unsigned char * out_vectorized_char = (unsigned char *) out_vectorized;
  
  Q1_routine_5(out_correct_char, b_char, c_char, (size*4 - 2));
  Q1_vectorized_5(out_vectorized_char, b2_char, c2_char, (size*4 - 2));
