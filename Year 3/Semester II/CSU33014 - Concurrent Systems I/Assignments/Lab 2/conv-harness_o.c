...
/* ##################################################################### STUDENT CODE ##################################################################### */
/* student version of multichannel_conv written */
// gcc conv-harness.c -o conv-harness -O3 -msse4 -fopenmp && ./conv-harness 256 256 5 32 32
// gcc conv-harness.c -o conv-harness -O3 -fopenmp && 
// ./conv-harness 512 512 1 512 512 && 
// ./conv-harness 512 512 3 512 512 && 
// ./conv-harness 512 512 5 512 512 && 
// ./conv-harness 512 512 7 512 512
void stu_mulchan_conv(float ***image, int16_t ****kernels,
	...
	double sum = 0.0; // instantiate variable outside loop
	#pragma omp parallel for private(m,c, wh, x, y) reduction(+:sum) collapse(2)
	/* 
	parallelise for loop
	privatise variables
	reduce sum to individual threads and connect together when assigned
	collapse outer two for loops
	 */
	for (m = 0; m < nkernels; m++) {
		for (wh = 0; wh < width*height; wh++) {
			...
			for (c = 0; c < nchannels; c++) {
				for (x = 0; x < kernel_order; x++) {
					for (y = 0; y < kernel_order; y++) {
						sum += image[wh/height + x][wh%height + y][c] * kernels[m][c][x][y]; // array accesses (read)
					}
				}
				output[m][wh/height][wh%height] = sum; // array access (write)
			}
		}
	}
}
				  ...
				  int kernel_order)
{
	...
	stu_mulchan_conv(image, kernels, output, width,
					  height, nchannels, nkernels, kernel_order);
}


long long run_and_avg(struct timeval start, struct timeval stop, 
					float ***image, int16_t ****kernels, float ***output, float ***control_output,
				  	int width, int height, int nchannels, int nkernels,
				  	int kernel_order,
					int iterations, int use_student) {
	long long avg = 0, mul_time = 0;
	int i;
	for (i = 0; i < iterations; i++) {
		...
		mul_time = (stop.tv_sec - start.tv_sec) * 1000000L +
				(stop.tv_usec - start.tv_usec);
		avg += mul_time;
		//if (use_student)
		//printf("Student conv time (iteration %d): %lld microseconds\n", i+1, mul_time);
		//else
		//printf("Original conv time (iteration %d): %lld microseconds\n", i+1, mul_time);
	}
...
int main(int argc, char **argv)
{
	...
	int iterations = 5;
	mul_time = run_and_avg(start_time, stop_time, image, kernels, output, control_output, width, height, nchannels, nkernels, kernel_order, iterations, 0);
	printf("Original conv time (avg of %d): %lld microseconds\n", iterations, mul_time);
	mul_time = run_and_avg(start_time, stop_time, image, kernels, output, control_output, width, height, nchannels, nkernels, kernel_order, iterations, 1);
	printf("Student conv time (avg of %d): %lld microseconds\n", iterations, mul_time);
