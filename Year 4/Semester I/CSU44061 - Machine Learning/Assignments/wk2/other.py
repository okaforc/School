...
VALUES = df.iloc[:, 2]
# print(df.head())
X = np.reshape(df.iloc[:, 0][VALUES==-1].to_numpy(), (-1, 1))
y = np.reshape(df.iloc[:, 1][VALUES==-1].to_numpy(), (-1, 1))
# X = np.arange(0, 1, 0.01).reshape(-1, 1) # xaxis values
# y = 10*(X**2) + np.random.normal(0.0, 1.0, X.size).reshape(-1, 1) # yaxis values
kf = KFold(n_splits=5)
