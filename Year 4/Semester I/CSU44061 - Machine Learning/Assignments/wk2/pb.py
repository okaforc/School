...
for Ci in Ci_range:
    ...
    # linear svc model
    model = LinearSVC(C=Ci, dual = False)
    ...
    # training data
    plt.scatter(Xtrain[:,0][ytrain == 1], Xtrain[:,1][ytrain == 1], marker=".", color="red", label="pos")
    plt.scatter(Xtrain[:,0][ytrain == -1], Xtrain[:,1][ytrain == -1], marker=".", color="green", label="neg")
